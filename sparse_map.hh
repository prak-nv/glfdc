...
// Paper: “An Efficient Representation for Sparse Sets” 1993 Preston Briggs , Linda Torczon
// http://www.cs.rice.edu/MSCP/papers/loplas.sets.ps.gz
// Also well described https://research.swtch.com/sparse
//
...
public:
  ...
  explicit sparse_map(std::size_t size) : sparse_(size)
  {
  ...
  bool has(std::size_t k) const noexcept
  {
    ...
    return sparse_[k] < dense_.size() && dense_[sparse_[k]].first == k;
  }

  std::optional<std::size_t> find(std::size_t k) const // O(1)
  {
    if (!has(k))
      return std::nullopt;
  ...
  // Precondition of this function is that there is no existing mapping for key k. 
